...
# -*- coding: utf-8 -*-
"""
Random Forest classifier on Angular Gyrus channels (left + right) for
spatial-attention decoding.

Trains a per-subject Random Forest restricted to Angular Gyrus channels with
PCA-based feature selection and nested cross-validation. Used to test whether
a focal AG ROI suffices for decoding (Figure 7) compared to the all-channels
variant.
...
# Get channels for Left AND Right Angular Gyrus
target_roi_L = 'Left-AngGyrus (39)'
target_roi_R = 'Right-AngGyrus (39)'
target_roi = 'LR Angular Gyrus'  # combined label for reporting

missing = []
if target_roi_L not in roi_dict:
    missing.append(target_roi_L)
...
target_channels_L = roi_dict[target_roi_L]
...
#%% Helper function for ROI filtering
def filter_to_roi_channels(run_data, target_channels, min_channels=5):
    """
    Filter run data to only include target ROI channels.
    
    Parameters:
    - run_data: recording data for one run
    - target_channels: list of channel labels for the target ROI
    - min_channels: minimum number of channels required to proceed
    
    Returns:
    - filtered_run_data: run data with only ROI channels
    - available_channels: list of channels that were actually found
    - skip_subject: boolean indicating whether to skip this subject
    """
    # Get available channels in the data
    data_channels = run_data["conc_p_tddr_filt"].channel.values
    
    # Find intersection of target channels and available channels
    available_channels = [ch for ch in target_channels if ch in data_channels]
    ...
    return run_data, available_channels, skip_subject
...
import pandas as pd
def load_subject_ag_channels(subj_id: str, condition: str):
    """Load previously extracted Angular Gyrus channels for a subject/condition.

    Returns a list of channel names filtered by fold presence if file exists.
...
# Build run‐level labels & recordings
    ...
    if USE_SUBJECT_SPECIFIC_AG_CHANNELS:
        ...
        roi_channel_info = {
            ...
            'run1_available': [c for c in subj_channels if c in run1_all],
            'run2_available': [c for c in subj_channels if c in run2_all],
            'common_channels_used': common.tolist(),
            'channel_selection_method': 'subject_specific_union_no_dprime'
        }
    else:
        ...
        roi_channel_info = {
            ...
            'run1_available': channels1,
            'run2_available': channels2,
            'common_channels_used': common.tolist(),
            'channel_selection_method': 'predefined_roi_no_dprime'
        }
    
    # Verify these common channels actually exist in both runs
    assert all(ch in run1_full["conc_p_tddr_filt"].channel.values for ch in common), "Common channels missing in run1"
    assert all(ch in run2_full["conc_p_tddr_filt"].channel.values for ch in common), "Common channels missing in run2"
    ...
    # Save ROI channel metadata for reproducibility
    with open(os.path.join(out_dir, "roi_channel_info.json"), "w") as fp:
        json.dump({
            "roi_filtering_applied": True,
            "target_roi": roi_channel_info['target_roi'],
            "expected_channels_count": len(roi_channel_info['expected_channels']),
            "expected_channels": roi_channel_info['expected_channels'],
            "run1_available_count": len(roi_channel_info['run1_available']),
            "run1_available_channels": roi_channel_info['run1_available'],
            "run2_available_count": len(roi_channel_info['run2_available']),
            "run2_available_channels": roi_channel_info['run2_available'],
    ...
    with open(os.path.join(out_dir, 'dprime_pca_summary.json'), 'w') as fp:
        json.dump({
            ...
            'use_subject_specific_ag': USE_SUBJECT_SPECIFIC_AG_CHANNELS,
            'channel_selection_method': roi_channel_info.get('channel_selection_method','unknown'),
            ...
            'roi_channel_info': roi_channel_info
        }, fp, indent=2)
#%%
