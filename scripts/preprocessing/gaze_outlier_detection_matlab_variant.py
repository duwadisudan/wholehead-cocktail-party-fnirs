"""
Gaze outlier detection — MATLAB-port variant (Neon eye tracker).

Python re-implementation of the legacy MATLAB gaze outlier pipeline for
direct cross-validation with the original. Reads aligned physio + events
TSVs, interpolates missing values, lowpass-filters at 2 Hz, converts to
visual degrees, baseline-corrects, epochs, and flags outlier trials by
...
RUNS = [
    ("overt",   "01", "overt_run-01"),
    ("overt",   "02", "overt_run-02")
]
...
MATLAB_GAZE_DIR = os.path.join(SNIRF_BASE, "derivatives", "matlab_gaze_subjects")
...
OUTLIER_PARAMS = {
    ...
    "covert": {
        ...
        "expected_right":  0,
        "tol":             20,
        "analysis_start_s": 1,  # from epoch start (2s before onset)
        "analysis_end_s":   4,
        "check_sign":      False,
    },
    "control": {
...
def process_run(subj, task, run):
    ...
    # Publication style (matches table_maker_scatter_overt_only_pub_latency_CI.py)
    plt.rcParams.update({
        'font.size': 16,
    ...
    ax1.axvline(0, color='gray', linestyle=':', linewidth=1.5, label='Onset')

    ax1.set_xlabel("Time from onset (s)", fontsize=18, fontweight='bold')
    ax1.set_ylabel("Gaze X (degrees)", fontsize=18, fontweight='bold')
    ax1.set_title(f"{base}  |  outliers: {n_outliers}/{len(onsets)}  "
                  f"(expected L={params['expected_left']}° R={params['expected_right']}° "
                  f"tol={params['tol']}°)", fontsize=16)
    ...
    ax1.legend(handles=legend_handles, loc='upper right', fontsize=13,
               frameon=True, edgecolor='black', framealpha=0.95)
    plt.tight_layout()

    # Save to gaze_pruning_neon (original location)
    subj_out = os.path.join(OUT_DIR, f"sub-{subj}")
    os.makedirs(subj_out, exist_ok=True)
    fig_path = os.path.join(subj_out, f"{base}_gaze_outliers.png")
    plt.savefig(fig_path, dpi=200, bbox_inches='tight', facecolor='white', edgecolor='none')
    print(f"  Saved figure: {fig_path}")

    # Save high-quality PNG, PDF, SVG to matlab_gaze_subjects
    matlab_subj_out = os.path.join(MATLAB_GAZE_DIR, f"sub-{subj}")
    os.makedirs(matlab_subj_out, exist_ok=True)
    for ext, kwargs in [
        ("png", {"dpi": 300, "bbox_inches": "tight", "facecolor": "white", "edgecolor": "none"}),
        ("pdf", {"bbox_inches": "tight", "facecolor": "white", "edgecolor": "none"}),
        ("svg", {"bbox_inches": "tight", "facecolor": "white", "edgecolor": "none"}),
    ]:
        out_path = os.path.join(matlab_subj_out, f"{base}_gaze_outliers.{ext}")
...
if __name__ == "__main__":
    ...
    os.makedirs(MATLAB_GAZE_DIR, exist_ok=True)
