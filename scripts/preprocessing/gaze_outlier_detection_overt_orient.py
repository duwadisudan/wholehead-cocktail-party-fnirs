"""
Gaze outlier detection — overt-orient condition (Neon eye tracker).

Reads the aligned physio + events TSVs for the overt-orient task,
interpolates missing values, lowpass-filters at 2 Hz, converts pixel gaze
to visual degrees, baseline-corrects, epochs around stimulus onset, and
flags trials whose sub-window gaze exceeds the outlier threshold. Outputs
per-run CSV of outlier trial indices and a diagnostic figure.
...
OUTLIER_PARAMS = {
    "overtorient": {
        ...
        "expected_right":  30,
        "tol":             20,
        "analysis_start_s": 2.0,   # seconds post-onset (after saccade lands)
        "analysis_end_s":   15.0,   # seconds post-onset (steady fixation)
...
def process_run(subj, task, run):
    ...
    # Plot a window that fits inside the available epoch samples.
    plot_pre  = 1.0
    plot_post = 16.0
    pre_skip  = int((PRE_CUE - plot_pre) * SAMPLING_RATE)
    n_plot = int((plot_pre + plot_post) * SAMPLING_RATE)
    plot_samples = min(n_plot, n_epoch - pre_skip)
    time_vec = np.arange(plot_samples) / SAMPLING_RATE - plot_pre
    ...
    for idx, ep, is_out in epochs_left:
        if not is_out:
            ax1.plot(time_vec, ep[pre_skip:pre_skip + plot_samples],
