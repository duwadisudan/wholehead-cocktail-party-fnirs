"""
Group-level Brodmann ROI hemodynamic response — covert condition only (supplementary).

Variant of the Figure 2 group-level ROI script restricted to the covert
attention condition.
...
#%%
# Covert — skip reload if data is already in memory (interactive sessions)
if 'rec_covert' in dir() and rec_covert is not None:
    print("rec_covert already in memory — skipping reload.")
elif flag_load_preprocessed_data:
    rec_covert, chs_covert = _load_flavor('covert')
else:
    print("Running load and preprocess for covert — flag_load_preprocessed_data=False")
    rec_covert, chs_covert = pf.preprocess_batch(cfg_dataset, cfg_preprocess)
    if flag_save_preprocessed_data:
        import gzip as _gz
        _save_dir = Path(rootDir_saveData) / f"preprocessed_covert_snr_{cfg_prune['snr_thresh']}"
        _save_dir.mkdir(parents=True, exist_ok=True)
        for s_idx, subj_id in enumerate(cfg_dataset['subj_ids']):
            with _gz.open(_save_dir / f"rec_subj_{subj_id}.pkl", 'wb') as f:
                pickle.dump(rec_covert[s_idx], f, protocol=pickle.HIGHEST_PROTOCOL)
            with _gz.open(_save_dir / f"chs_pruned_subj_{subj_id}.pkl", 'wb') as f:
                pickle.dump(chs_covert[s_idx], f, protocol=pickle.HIGHEST_PROTOCOL)
        print(f"Saved preprocessed covert data to: {_save_dir}")

#%%
# Apply gaze + behavioural correctness trial filter
if flag_filter_trials:
    apply_trial_filter(rec_covert, cfg_dataset['subj_ids'],
                       cfg_dataset['file_ids'], cfg_dataset['root_dir'], 'covert')
else:
...
GRID_WIDTH_MM  = 180
GRID_HEIGHT_MM = 210
GRID_WIDTH_IN  = GRID_WIDTH_MM  / 25.4
GRID_HEIGHT_IN = GRID_HEIGHT_MM / 25.4

PUB_FONT_SIZE = 8   # minimum 7 pt for publication

plt.rcParams.update({
    'font.family'      : 'sans-serif',
    'font.sans-serif'  : ['Arial', 'Helvetica', 'DejaVu Sans'],
    'font.size'        : PUB_FONT_SIZE,
    'axes.labelsize'   : PUB_FONT_SIZE+2,
    'xtick.labelsize'  : PUB_FONT_SIZE,
    'ytick.labelsize'  : PUB_FONT_SIZE,
    'legend.fontsize'  : PUB_FONT_SIZE - 1,
...
def _plot_hemisphere_grid(rois, hemisphere_label, robust_results, save_path):
    """
    Plot all ROIs for one hemisphere as subplots in a single figure.
    Artboard: 180 mm × 210 mm.  Saves PDF and SVG only.
    """
    n = len(rois)
    n_cols = 3
    ...
    fig, axes = plt.subplots(
        n_rows, n_cols,
        figsize=(GRID_WIDTH_IN, GRID_HEIGHT_IN),
        squeeze=False,
    )
    ...
    colors = {
        ...
        'HbO_right' : [1,   0.27, 0   ],
        'HbR_left'  : [0,   0,    0.8 ],
        'HbR_right' : [0,   0.39, 0.8 ],
    }
    sem_alpha              = 0.2
    line_width             = 1.0
    significant_line_width = 3.0
    events       = [0, 2, 5]
    event_colors = ["black", "green", "orange"]

    condition_name = 'Covert'
    ...
    for idx, roi in enumerate(rois):
        ...
        if condition_name not in robust_results:
            ax.text(0.5, 0.5, 'No data', ha='center', va='center',
                    transform=ax.transAxes, fontsize=PUB_FONT_SIZE)
            ax.set_title(roi, fontsize=PUB_FONT_SIZE, fontweight='bold')
            continue
        ...
        sig_mask   = results.get('sig_mask')
        if sig_mask is not None and roi in sig_mask.ROI.values:
            sig_mask = sig_mask.sel(ROI=roi)
        else:
            sig_mask = None
        ...
        for chromo in group_mean.chromo.values:
            for trial_type in group_mean.trial_type.values:
                ...
                if sig_mask is not None:
                    ct_sig = sig_mask.sel(chromo=chromo, trial_type=trial_type)
                    if ct_sig.any():
                        lw = significant_line_width

                ax.plot(t, m, color=color, linewidth=lw, alpha=0.9)
                ax.fill_between(t, m - se, m + se, color=color, alpha=sem_alpha)

        for x, c in zip(events, event_colors):
            ax.axvline(x, linestyle="--", color=c, linewidth=0.8, alpha=0.7)

        ax.set_xlim(t.min(), t.max())
        ax.set_xticks([0, 5, 10])
        ax.grid(True, linestyle=":", alpha=0.3, linewidth=0.5)

        # ROI name as subplot title
        ax.set_title(roi, fontsize=PUB_FONT_SIZE, fontweight='bold')
    ...
    stem = f"HRF_grid_{hemisphere_label}"
    fig.savefig(save_path / f"{stem}.png", dpi=300, bbox_inches='tight')
    fig.savefig(save_path / f"{stem}.pdf", dpi=300, bbox_inches='tight')
    fig.savefig(save_path / f"{stem}.svg", bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved {stem}.pdf / .svg  ({n} ROIs, {n_rows}×{n_cols} grid)")


# %%
# SAVE GRID FIGURES — LEFT AND RIGHT HEMISPHERES

save_dir = _PATHS.group_avg_results_root / "figures_BA_snr_0_covert_only_pub_filtered_trials"
save_dir.mkdir(parents=True, exist_ok=True)
print(f"\nSaving grid figures to: {save_dir}")

_plot_hemisphere_grid(left_rois,  "Left_Hemisphere",  robust_results, save_dir)
_plot_hemisphere_grid(right_rois, "Right_Hemisphere", robust_results, save_dir)

print("\nGrid publication plots complete!")
print("   Thick lines = significant | Thin lines = non-significant")
print(f"   Artboard: {GRID_WIDTH_MM}×{GRID_HEIGHT_MM} mm, {PUB_FONT_SIZE} pt sans-serif")
