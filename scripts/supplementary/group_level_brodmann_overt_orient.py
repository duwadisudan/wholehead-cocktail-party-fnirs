"""
Group-level Brodmann ROI hemodynamic response — overt-orient condition (supplementary).
...
# This script is single-purpose for the overt-orient task; condition is fixed
...
# Cohort of subjects with overt-orient runs. Override via run.yml.
# Note: 'subjects: test' resolves to sub-10, which does not have overtorient
...
rootDir_saveData = str(_PATHS.derivatives_root / "overtorient_brodmann_snr0") + os.sep
flag_save_preprocessed_data = True  # save rec/chs if we preprocess now
flag_run_type = 'overtorient'  # this script is overt-orient only by design
...
if flag_run_type.lower() == 'overtorient':
    ...
    raise ValueError(f"flag_run_type must be 'overtorient', got {flag_run_type!r}")
...
cfg_blockavg = {
    ...
    'trange_hrf_stat' : [4, 8],
    'stim_lst_hrf' : ['overtorient Left', 'overtorient Right'],
    'flag_run_type'      : flag_run_type,   # 'overtorient'
...
"""
overtorient-only analysis: load or preprocess overtorient runs (3 runs),
...
# Load or preprocess overtorient data
if flag_load_preprocessed_data:
    try:
        rec_overtorient, chs_overtorient = _load_flavor('overtorient')
        print("Loaded saved overtorient rec/chs")
    except FileNotFoundError:
        print("Saved overtorient rec/chs not found. Preprocessing now...")
        rec_overtorient, chs_overtorient = pf.preprocess_batch(cfg_dataset, cfg_preprocess)
        if flag_save_preprocessed_data:
            outdir = Path(rootDir_saveData)
            outdir.mkdir(parents=True, exist_ok=True)
            with gzip.open(outdir / 'rec_list_overtorient.pkl', 'wb') as f:
                pickle.dump(rec_overtorient, f, protocol=pickle.HIGHEST_PROTOCOL)
            with gzip.open(outdir / 'chs_pruned_subjs_overtorient.pkl', 'wb') as f:
                pickle.dump(chs_overtorient, f, protocol=pickle.HIGHEST_PROTOCOL)
else:
    rec_overtorient, chs_overtorient = pf.preprocess_batch(cfg_dataset, cfg_preprocess)
    if flag_save_preprocessed_data:
        outdir = Path(rootDir_saveData)
...
ba_overtorient = _blockavg_all_runs(rec_overtorient, stim_labels['overtorient'])
...
subj_roi_overtorient = roi_mean_per_subject(subj_avg_overtorient)
...
condition_name = 'overtorient'
subj_data = subj_roi_overtorient
...
def _save_timeseries_bundle(
    ...
    - condition_key: key in robust_results (e.g., 'overtorient')
    - condition_attr: string attr to store (e.g., 'overtorient')
    ...
    def _drop_scalar_reltime(da: xr.DataArray) -> xr.DataArray:
        # Remove stray scalar 'reltime' coordinates (from isel) that clash with dim reltime
    ...
    ds = xr.Dataset(
        ...
        attrs=dict(
            ...
            dataset_version="1.0",
            source_script="group_level_brodmann_overtorient.py",
        ),
    )
...
def plot_roi_group_robust(roi, robust_results, save_path=None):
    """
    Plot group average HRFs for a given ROI for overtorient.
    ...
    roi_found = False
    for condition_name in ['overtorient']:
...
save_dir = _PATHS.group_avg_results_root / "figures_brodmann_overtorient"
...
try:
    _save_timeseries_bundle(
        subj_roi_list=subj_roi_overtorient,
        robust_results=robust_results,
        condition_key="overtorient",
