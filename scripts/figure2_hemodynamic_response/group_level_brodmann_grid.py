"""
Group-level Brodmann ROI hemodynamic response grid (Figure 2).

For each ROI, computes the group mean ± SEM HbO/HbR time course across
subjects from the preprocessed per-subject derivatives, and renders the
multi-panel ROI grid layout with significance markers per ROI.
...
# Pipeline mode and condition come from config/run.yml. See require_run()
# above. mode='full' means rerun preprocessing from raw; condition selects
# the overt vs covert task ids below.
...
#%%
# Overt — skip reload if data is already in memory (interactive sessions)
if 'rec_overt' in dir() and rec_overt is not None:
    print("rec_overt already in memory — skipping reload.")
...
def _plot_hemisphere_grid(rois, hemisphere_label, robust_results, save_path):
    ...
    # Adjust height to keep subplots roughly square-ish given the column count
    fig_h = GRID_HEIGHT_IN * (n_rows / max(n_rows, 1))
    fig_h = max(fig_h, GRID_HEIGHT_IN * 0.6)  # floor so it doesn't get too short
    fig_h = min(fig_h, GRID_HEIGHT_IN)         # cap at artboard height
