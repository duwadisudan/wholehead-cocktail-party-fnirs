"""
MNI Registration Error Analysis: Compare subject channel positions to template.

This script:
1. Loads template MNI positions (where channels SHOULD be)
2. Loads subject-specific registered MNI positions (where channels ARE)
3. Computes registration error (distance from template) per channel per subject
4. Generates 2D and 3D visualizations showing error magnitude
5. Computes cross-subject variability statistics
"""
