...
def load_stability_group_csv(base_dir: Optional[str], condition: str, direct_csv: Optional[str] = None) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        # ensure stability_score exists: prefer existing; else compute from median or mean dprime
        if 'stability_score' not in df.columns:
            ...
            elif 'mean_dprime' in df.columns and 'stability_subject_frequency' in df.columns:
                df['stability_score'] = 0.6 * df['mean_dprime'].astype(float) + 0.4 * df['stability_subject_frequency'].astype(float)
        return df
    except Exception as e:
...
def merge_roi_tables(df_contrib: pd.DataFrame, df_stability: pd.DataFrame) -> pd.DataFrame:
    ...
    keep_cols_stab = ['roi_name', 'median_dprime', 'mean_dprime', 'stability_subject_frequency', 'stability_score']
...
def main():
    ...
    views = [
        {
            ...
            'title': 'Top 30 ROIs by contrib_mean_importance (X = stability_score)'
        },
        {
            'name': 'stable_top20',
    ...
    for cond in conditions:
        ...
        for view in views:
            ...
            # robust fallback preference for X metric
            mx = view['metric_x']
            if mx not in merged_top.columns:
                for alt in ['stability_score', 'median_dprime', 'mean_dprime']:
                    if alt in merged_top.columns:
                        mx = alt
                        break
            my = view['metric_y']
